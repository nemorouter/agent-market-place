...
| [`ask-genie`](./ask-genie) | **Our dogfood** — the support agent on `nemorouter.ai`. Instance of the customer-service-agent runtime, but uses the **existing Nemo Supabase** in a dedicated **`nemo_amp_db`** schema (Rule #12 — never `public`). Replaces the legacy support agent. | 🧩 instance (config + docs + schema) |
